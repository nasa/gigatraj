...
int main() 
{
    ...
    if ( filter.query("count", &count ) ) {
       if ( count != 2 ) {
          cerr << "array filter query returned  " << count << " instead of 2" << endl;
          exit(1);           
       }
    } else {
       cerr << "array filter query failed " << endl;
       exit(1);        
    }
    // try it with the filter negated
    filter.negation(true);
    filter.apply( p_array, n );
    if ( filter.query("count", &count ) ) {
       if ( count != (n - 2) ) {
          cerr << "array negated filter query returned  " << count << " instead of " << n - 2 << endl;
          exit(1);           
       }
    } else {
       cerr << "array negated filter query failed " << endl;
       exit(1);        
    }
    ...
    if ( filter.query("count", &count ) ) {
       if ( count != 2 ) {
          cerr << "vector filter query returned  " << count << " instead of 2" << endl;
          exit(1);           
       }
    } else {
       cerr << "vector filter query failed " << endl;
       exit(1);        
    }
