...
// Tests the Filter_Advance class
...
int main() 
{
    ...
    static PGenFile gen;
    std::string fname;
    Filter_Advance advancer;
    
    dt = 0.01 ; // 15-minute time step
    
    advancer.timestep(dt);
    if ( mismatch( dt, advancer.timestep(), dt*0.01 ) ) {
       cerr << "Failed to set the timestamp " << dt << " vs " << advancer.timestep() << endl;
       exit(1);
    }
